...
- **19 templates you can actually download** — including **8 real Excel workbooks with live formulas**, data-validation dropdowns and conditional formatting. RPN, weighted scores, Process Cycle Efficiency, kappa, sigma level and ROI all calculate themselves. Every workbook has a "how to use this" tab, a worked example row, and a legend telling you which cells to fill in. Every template also has an **Email version** — one click renders it as email-safe HTML you can paste into Outlook or Gmail with formatting intact, with each calculated cell showing its formula underneath the value.
