...
| Chart type justification | *Proportion at ~5,100 billing tickets/week — the whole queue; σ_z = 2.08, so an ordinary p-chart is too tight. Its limits would be 15.7% / 12.7%, and five of the twelve weekly rates in section 3 sit outside them — one above and four below, with one special cause behind it. Between-week variation this far above the binomial is what Laney is for* |
...
| UCL / LCL | *17.2% / 11.2%* |
| σ_z (if attribute chart) | *2.08 — computed from the twelve weekly rates in section 3 and their denominators beside them, as the average moving range of their z-scores ÷ 1.128. The limits follow: 1.4663 × 2.08 = a half-width of 3.05 points, 1.4663 already being the ordinary 3σ half-width and not a 1σ one. Over 1.2, so Laney is the right chart. The 1.98 this row used to carry was read backwards off the limits themselves, so it corroborated nothing* |
...
| Statistic | Value |
|---|---|
| n | *61,400 tickets over the 12-week window (~5,100/week), week by week 6,400 × 4 in January, 4,947 × 3 across the pricing weeks from 1 February, then 4,192, 4,192, 4,192, 4,192 and 4,191 — the whole Billing queue; every statistic in this section is the queue's, not the adjustment population's* |
| Mean | *14.2% — the queue rate (OD-BIL-004 v2), 8,719 reopens across all 61,400 tickets. The twelve weekly rates average 14.1% unweighted, because January carries a quarter more volume at a higher rate; the ticket-weighted figure is the process average and the one every other document cites, and the standard deviation below is the spread of the weekly points about it* |
| **Median** | *14.0%* |
| Standard deviation | *1.8 points, on the twelve weekly rates this section is cut on: 14.9, 14.7, 14.5, 14.3, 13.8, 14.0, 18.9, 14.0, 12.6, 12.5, 12.4 and 12.2 (%, w/c 2026-01-05 onward). Every figure in this section is computed from that series. The 1.4 points this row used to carry was not reachable by any series: one week at 18.9% against a mean of 14.2% puts the floor at 1.48 on its own* |
| p10 / p50 / p90 / p95 | *12.4% / 14.0% / 14.9% / 16.7%, by linear interpolation between order statistics. The 12.3 / 16.2 / 16.9 this row used to carry is not reachable from this series under any percentile convention: p90 of 16.2% needs a tenth-largest week of 16.2%, and the tenth largest is 14.7%* |
| Skewness | *1.56 — the moment coefficient m₃/m₂^1.5; Excel's SKEW() applies the sample bias correction and returns 1.79 on the same twelve numbers, so say which you used. One week four points clear of the rest is what either of them is reporting. The 0.31 this row used to carry described a series with no release in it* |
| Distribution shape | *Not symmetric: eleven weeks between 12.2% and 14.9% and one at 18.9%. The mean-median gap understates it — 0.07 points on the weekly series — because one point in twelve moves a mean very little. The skewness and the maximum are what show it* |
| Normality assessment (probability plot, not just the p-value) | *Probability plot linear across eleven weeks with the release week clear of the line; Anderson-Darling A² = 0.81 on the twelve published rates, p = 0.02. The departure is that one point, not the shape of the rest — which is why it is treated as a special cause in section 2 rather than as a transformation problem* |
...
| **Capability index value** | *Ppu −1.15 — (8.0% − 14.2%) ÷ (3 × 1.8 points), on the mean and standard deviation section 3 reports for this series. It is negative because the queue's mean sits above the only limit there is; the positive index this row used to carry is not derivable from any series in this document* |
| % outside specification (observed) | *100% — all 12 weeks sit above the 8.0% target. The lowest weekly point, 12.2%, is above the 11.2% LCL in section 2 and p10 is 12.4% (section 3), so no week is close to it. The minority-of-weeks figure this row used to carry is not derivable from that series* |
| % outside specification (fitted) | *>99.9% — at a mean of 14.2% and a standard deviation of 1.8 points, the 8.0% target is 3.4 standard deviations below the mean. The near-coin-flip figure this row used to carry is not derivable from that series* |
...
| *List price change on the mid-tier broadband plan* | *2026-02-01* | *Disputed-charge volume up ~18% for three weeks; queue reopen rate flat at 13.9% across the first two of them — the change moved the denominator, not the process. The third is the release week two rows down, and nothing about the pricing change can be read off it* | *Included. Pricing changes are a standing feature of the business; the baselined metric is a rate, so volume alone does not distort it* |
...
| *v1.1* | *`<same date>`* | *Section 1 restored to describing the extract it actually holds. Its metric and operational-definition rows name OD-BIL-004 v2, the whole-queue definition every row of that table is cut on; the job, the record count and the snapshot were always the queue's. The project's Y has no query, no extract date and no snapshot recorded here, and none traced anywhere in the pack (06-data-lineage.md:69), so the snapshot row now carries that blank instead of implying a snapshot exists.* |
| *v1.1* | *`<same date>`* | *Section 4 recomputed against the series it is cut on. Ppu was positive and the out-of-spec rates a minority of weeks observed and about six in ten fitted — CHANGELOG.md keeps the exact figures, which this document may no longer repeat; on section 3's mean of 14.2% and standard deviation of 1.8 points against the 8.0% target, Ppu is −1.15 and every one of the 12 weeks is above target. DPO, Z and sigma keep their values — 0.142 ÷ 5 = 0.0284 gives Z 1.90 and 3.40 with the shift — and now say which denominator they are on, so the section no longer reads as three rival out-of-spec rates.* |
...
| *v1.1* | *`<same date>`* | *Sections 2 and 3 recomputed from a published weekly series. The standard deviation read 1.4 points, which no series can produce: one week at 18.9% against a mean of 14.2% puts the floor at 1.48 on its own, and and p90 was quoted at 16.2% when the tenth-largest week is 14.7%. Both were assertions with no data behind them, and so were the percentiles, the skewness and the σ_z, which was read backwards off the limits it was meant to justify. The twelve weekly rates now sit in the standard-deviation row and every figure in sections 2–4 is computed from them: SD 1.8 points, p10/p90/p95 12.4 / 14.9 / 16.7, skewness 1.56, σ_z 2.08, limits 17.2% / 11.2%, Ppu −1.15. The series reproduces the figures this document already stated — the 14.2% queue rate, the 14.0% median, January at 14.6%, the 18.9% release week, exactly one point above the UCL and the 15.7% / 12.7% ordinary limits — so nothing outside these two sections moves. One row of section 6 did move: the pricing change's "flat at 13.9%" now says which two weeks it covers, because the third of its three weeks is the release week and 13.9% was never true across all three.* |
