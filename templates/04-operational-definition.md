...
| **Refresh cadence** | *`<not established for OD-BIL-004-ADJ>`. The nightly 02:00 UTC build is the OD-BIL-004 v2 pipeline's cadence (its dbt and warehouse runs at templates/06-data-lineage.md:28-30) and was inherited by this row; templates/06-data-lineage.md:69 records this metric as having no refresh at all. The "restated for 3 days as late events land" this row used to carry is in no document in the pack — no restatement window is recorded for either metric, and hop 3 and hop 4 state run times only. Do not quote a schedule for a number currently produced by an untraced query — same owners, same deadline* |
| **Metric is final at** | T + ___ *(not statable for OD-BIL-004-ADJ until the refresh cadence above exists — templates/06-data-lineage.md:69)* |
| **Owner** | *A. Okafor, Billing Ops Manager — owns the definition, not just the number* |
| **Known limitations** | *No channel is excluded from this definition — all three are in scope (templates/02-sipoc.md:42, templates/06-data-lineage.md:62). The legacy filter that drops chat sits on the Looker Ops weekly tile at hop 6 (templates/06-data-lineage.md:31, :70), downstream of everything this page names as a source, so it is that tile's defect and not this metric's. The limitation that does bite is upstream of the number: no query for OD-BIL-004-ADJ is traced anywhere in the pack (templates/06-data-lineage.md:69), and the merged-ticket restatement has never been applied to this population (templates/06-data-lineage.md:70) — nor does the pack record its size, the "13.9% to 14.2%" once claimed for it being struck on that same line as sourced nowhere, so the 137 / 966 cut may move by `<effect of the merged-ticket restatement on OD-BIL-004-ADJ>` — Data Eng and Analytics to restate it when they trace the query, before this definition is re-signed* |
| **Related metrics it must reconcile with** | *OD-BIL-004 v2 — the 7-day reopen rate on all billing tickets, same 168-hour rule, whole queue, census, also 14.2% (templates/06-data-lineage.md:61). Same number, different quantity: context only. That 14.2% is quoted for 1-31 Mar. templates/06-data-lineage.md:53 calls that a comparison cut and not a baseline window; whether it is the same month as this metric's baseline month is unknown, because the pack never names that month — see the denominator row. The two reconcile by size, not by value — 11,592 in-scope adjustments a year (itself the 12x extrapolation flagged in the denominator row) against the queue's 266,000 (templates/01-project-charter.md:48) is 4.4% of it, so closing every reopen in this population moves the queue rate from 14.2% to 13.58%, a 0.62-point move. Any claim that this metric shifts the queue rate by more than that is arithmetically impossible. The Ops weekly reopen tile reads 12.4%, 1.8 points under the queue figure (templates/06-data-lineage.md:59) — that gap reconciles OD-BIL-004 v2, not this metric* |
